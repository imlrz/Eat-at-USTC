...
def adddishes(request):
    if request.method == 'POST':
        ...
        if not resta_ID or not dish_name:
            return render(request, 'catalog/add_dishes.html', {'resta_ID': resta_ID, 'errors': ['餐厅与菜名必填']})
        if not image:
             image = 'dishes/404.png'

        # 菜品已存在
        if DISH.objects.filter(resta_ID=resta_ID, dish_name=dish_name).exists():
            return render(request, 'catalog/add_dishes.html', {'resta_ID': resta_ID, 'errors': ['菜品已存在']})
...
def editrestaurants(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, "catalog/edit_restaurants.html", {'restaurant': restaurant, 'errors': e.messages})
        except IntegrityError as e:
            print(f'ERROR: Integrity error - {e}')
            return render(request, "catalog/edit_restaurants.html", {'restaurant': restaurant, 'errors': [str(e)]})
...
def register_manager(request):
    if request.method == 'GET':
        return render(request, 'registration/register_manager.html')
        ...
        # 用户不存在
            ...
            # 确认 user.is_staff 是否已成功保存
            user.refresh_from_db()
            if user.is_staff:
                return redirect('managerlogin')
            else:
                return render(request, 'registration/register_manager.html', {'errors': ['用户创建失败，请重试']})
